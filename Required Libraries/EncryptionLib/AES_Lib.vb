...
Public Class AES_Lib
    ' define the AES
    Private m_aes As New AesCryptoServiceProvider
    ...
    Public Function Encrypt(ByVal input() As Byte) As Byte()
        Return Transform(input, m_aes.CreateEncryptor(m_key, m_iv))
    End Function

    Public Function Decrypt(ByVal input() As Byte) As Byte()
        Return Transform(input, m_aes.CreateDecryptor(m_key, m_iv))
    End Function

    Public Function Encrypt(ByVal text As String) As String
        Dim input() As Byte = m_utf8.GetBytes(text)
        Dim output() As Byte = Transform(input, _
                        m_aes.CreateEncryptor(m_key, m_iv))
    ...
    Public Function Decrypt(ByVal text As String) As String
        Dim input() As Byte = m_utf8.GetBytes(text)
        Dim output() As Byte = Transform(input, _
                         m_aes.CreateDecryptor(m_key, m_iv))
        Return m_utf8.GetString(output)
    End Function
