...
Namespace Library
    Public NotInheritable Class PackagesLib
        Private Sub New()
        End Sub
        ...
        Public Shared Sub RegisterModule(ByVal enviroment As LuaTable)
            ...
            enviroment.SetNameValue("Packages", [module])
            [module].SetNameValue("_G", enviroment)
            currentModule = [module]
        End Sub

        Public Shared Sub RegisterFunctions(ByVal [module] As LuaTable)
            [module].Register("Create", New LuaFunc(Function(values As LuaValue()) As LuaValue
                                                        Return New LuaUserdata(New PackFileInfoCollection)
                                                    End Function))
            [module].Register("AddFile", New LuaFunc(Function(values As LuaValue()) As LuaValue
                                                         Dim p As PackFileInfoCollection = DirectCast(CType(values(0), LuaUserdata).Value, PackFileInfoCollection)
                                                         If (values.Count - 1) = 1 Then
                                                             p.AddFile(values(1).Value.ToString)
                                                         Else
                                                             p.AddFile(values(1).Value.ToString, values(2).Value.ToString)
                                                         End If
                                                         Return New LuaUserdata(p)
                                                     End Function))
            [module].Register("AddDirectory", New LuaFunc(Function(values As LuaValue()) As LuaValue
                                                              Dim p As PackFileInfoCollection = DirectCast(CType(values(0), LuaUserdata).Value, PackFileInfoCollection)
                                                              If (values.Count - 1) = 1 Then
                                                                  p.AddDirectory(values(1).Value.ToString)
                                                              Else
                                                                  p.AddDirectory(values(1).Value.ToString, values(2).Value.ToString)
                                                              End If
                                                              Return New LuaUserdata(p)
                                                          End Function))
            [module].Register("Pack", New LuaFunc(Function(values As LuaValue()) As LuaValue
                                                      Dim p As PackFileInfoCollection = DirectCast(CType(values(0), LuaUserdata).Value, PackFileInfoCollection)
                                                      LuaBoolean.From(Packages.Pack(p, values(1).Value.ToString))
                                                  End Function))
            [module].Register("Unpack", New LuaFunc(Function(values As LuaValue()) As LuaValue
                                                        Return LuaBoolean.From(Packages.Unpack(values(0).Value.ToString, values(1).Value.ToString))
                                                    End Function))
            Dim metaTable As New LuaTable()
            'metaTable.Register("__index", AddressOf Nothing)
